...
if [ "$STAGE" == "CI" ]; then
  ...
  if [ "$STAGE" == "CI" ]; then
    invokeCmd "sfdx force:source:deploy -p force-app -u $TARGET_SCRATCH_ORG_ALIAS -c -l RunLocalTests --apiversion $SFDX_API_VERSION --ignorewarnings true --wait 1000 --verbose"
# else
  # Push source
